...
- [ ] Crear directorio `db`.
- [ ] Escribir `db/schema.sql` con las tablas: `problems`, `test_cases`, `submissions`.
- [ ] Escribir `db/seed.sql` con datos de prueba (por ejemplo, el problema "Mario" de la Semana 1 en C).

### 3. Capa de Dominio (`internal/domain`)
- [ ] Definir entidad `Problem` (ID, Title, Description, Difficulty, Language, Week).
- [ ] Definir entidad `TestCase` (ID, ProblemID, InputData, ExpectedOutput, IsHidden).
- [ ] Definir entidad `Submission` (ID, ProblemID, Code, Status, ExecutionTimeMs, MemoryKb, SubmittedAt).
- [ ] Definir `ExecutionResult` (Stdout, Stderr, ExitCode, TimeMs, MemoryKb).
- [ ] Definir `CodeSnippet` (Code, Language).
...
- [ ] Implementar Casos de Uso (`internal/application/usecases`):
...
- [ ] Implementar repositorios SQLite (`internal/infrastructure/sqlite`):
  - `SQLiteProblemRepository`
  - `SQLiteTestCaseRepository`
  - `SQLiteSubmissionRepository`
- [ ] Implementar adaptador HTTP (`internal/infrastructure/http`):
  - Handlers para los endpoints usando Gin.
  - `GET /api/v1/problems`
  - `GET /api/v1/problems/:id`
  - `POST /api/v1/submissions`
- [ ] Implementar Motor de Ejecución Docker (`internal/infrastructure/docker`):
...
- [ ] Configurar `main.go` para inyectar dependencias:
  - Conectar a SQLite.
...
- Se puede hacer un `GET /api/v1/problems` y obtener la lista inicial.
- Se puede hacer un `POST /api/v1/submissions` con código en C válido y obtener status `Accepted`.
- Se puede hacer un `POST /api/v1/submissions` con código en C inválido y obtener status `Failed` (compilation error o wrong output).

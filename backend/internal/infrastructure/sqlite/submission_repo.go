...
func (r *SQLiteSubmissionRepository) FindByProblemId(problemId string) ([]domain.Submission, error) {
	rows, err := r.db.Query("SELECT id, problem_id, code, status, execution_time_ms, memory_kb, submitted_at FROM submissions WHERE problem_id = ? ORDER BY submitted_at DESC", problemId)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
